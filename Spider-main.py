...
#     spider.setsubpage(home_page,page_count,SUBPAGE_EXCEL) # 利用a标签提取大致网址并利用href标签提取具体网站导入新创建的excel当中
#     page_count += 1
#
# # 返回行数得知一共有几条二级页面网址
...
#
# # # 遍历网址，取出数据
# spider.creatdataexcel(NEW_CONFIRMED_CASES_DATA_EXCEL,NEW_ASYMPTOMATIC_INFECTIONS_DATA_EXCEL,NEW_SPECIAL_ZONES_DATA_EXCEL) #创建用来统计数据的excel
# page_count = 1
...
#         continue
#     else:
#         spider.new_confirmed_cases(data,row_count,blank_line,NEW_CONFIRMED_CASES_DATA_EXCEL) # 获取新增确诊人数数据
#         spider.new_asymptomatic_infections(data,row_count,blank_line2,NEW_ASYMPTOMATIC_INFECTIONS_DATA_EXCEL)
...
# 生成中国近一日各省市新增确诊人数
map = showdata.show_new_confirmed_data(NEW_CONFIRMED_CASES_DATA_EXCEL,NEW_SPECIAL_ZONES_DATA_EXCEL)
# 将多张图合成一个html进行展示
page = Page()
page.add(map,line1,line2)
